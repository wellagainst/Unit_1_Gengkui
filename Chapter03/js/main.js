...
//define a function that later used inside debugAjax to add the content into webpage
...
function debugAjax(){
    ...
    //get/retrive the geojson data from the server
	fetch("data/MegaCities.geojson")
        //only after the data has been successfully fetched, we then convert the data into json format
		.then(function(response){
            return response.json();
		})
        //only after the conversion has been finished, we assign the updated response to myData
        .then(function(response){
            myData=response;
            //check if myData is available or not
            console.log(myData)
            //call debugCallback function to add the json information on the webpage
            debugCallback(myData)
        })
        
    
};
//execute the script after the webpage completely loaded
